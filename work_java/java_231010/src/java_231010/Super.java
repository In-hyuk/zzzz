...
// 자식 클래스에서 객체 생성 시, 부모 클래스의 생성자를 우선 실행한 뒤에 자식 클래스 생성자 실행
class NoteBook extends Computer{
	// 멤버 변수 : name(기종), company(제조회사), weight(무게), angle(꺾이는 각도)
	// 상속을 함으로써, name, company 변수 생략
	...
	public NoteBook(String n, String c, double w, int a) {
		super(n, c);		// 매개변수 n 과 c 의 정보를 전송하면서, Computer(String, String)이 수행되고 있음
		this.weight = w;
		this.angle = a;
...
public class Super {
	public static void main(String[] args) {
		...
		Computer computer = new Computer(scan.next(), scan.next());
		NoteBook notebook = new NoteBook(scan.next(), scan.next(), scan.nextDouble(), scan.nextInt());
	}
}
