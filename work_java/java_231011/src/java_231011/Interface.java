...
// 추상클래스 발전 -> 인터페이스 개념
// 원칙적으로 자바에선 다중 상속 (여러 클래스를 상속) 지원X
...
// 인터페이스 통해서 객체 생성X
// 인터페이스 상속받아서, 다른 크래스에서 재정의/구체화를 진행
interface ani{
	// 필드 : "상수"의 형태로 선언 (변수 선언X)
	//		상수 -> 선언과 동시에 초기화, 값 변경X	
	// 멤버 함수(메소드) : 추상 메소드로만 구성
	// "모든 멤버들이 접근제어자가 반드시 public 이어야 한다."
...
class Puppy implements ani{
	// 인터페이스 Animal 에 있는, Print() 및 cry() 필수로 구현
	
	public void Print() {
		System.out.println("강아지입니다.");
	}
	public void cry() {
		System.out.println("월월월월ㅁㄴㅇㄹ");		
	}
}
...
public class Interface {
	public static void main(String[] args) {
		// "인터페이스 통해서는 객체 생성X"
		// Animal a = new Animal(); -> 에러O, 인터페이스로 객체 생성 시도
