...
class User implements Character{
	...
	public void attack() {
		...
		System.out.println( "데미지를 줬습니다. 남은 mp : " + this.mp);
	}
	// 메소드 : minus(int) -> 매개변수 값의 1/defense 값만큼 hp 감소
	//					    데미지를 받았다는 것을 출력함과 동시에, 남은 User hp 를 출력
	public void minus(int hit) {
		int attack = hit / this.defense;
		this.hp = this.hp - attack;
		System.out.println(attack + "만큼 데미지를 받았습니다. 남은 hp : " + this.hp);
	}
	public User(int hit, int d) {
		this.hit = hit;
		this.defense = d;
	}
}
...
public class Ex_ {
	public static void main(String[] args) {
		...
		while(user1.getMp() !=0 || user2.getMp() !=0) {
			System.out.print("user1의 공격! ");
			user1.attack();
			System.out.print("user2가 ");
			user2.minus(user1.getHit());
			if(user2.getHp() <= 0) {
				System.out.println("user1 승리!");
				break;
			}
			System.out.print("user2의 공격! ");
			user2.attack();
			System.out.print("user1이 ");
