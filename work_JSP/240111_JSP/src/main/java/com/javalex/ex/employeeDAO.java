...
public class employeeDAO {
	...
	public ResultSet SearchUser(String id, String pw) {
		...
		// conn 같은 경우에는 Connect()의 값을 유지해서 사용해야하기 때문에 null값으로 치가화하면 안됨
		ps=null;
		ResultSet result=null;
		try {
			String query="SELECT * FROM employee WHERE id = ? && passwd = ?";
			ps=conn.prepareStatement(query);	// ps 객체 초기화 (사용할 쿼리문 미리 적용)
			ps.setString(1, id);	// 첫번째 ?에 input_id 값을 삽입
			ps.setString(2, pw);
			result=ps.executeQuery();
		} catch(Exception e) {
			System.out.println("계정 탐색 에러");
		} 
		return result;
	}
	
	// employee DB : 아이디, 비밀번호, 이름
	// 예상할 수 있는 필요한 기능 : DB에 저장된 아이디 비번 중, 입력으로 받은 아이디 비번이 있는지
	public Boolean check_Account(String input_id, String input_pw) {
		...
		try {
			...
			// 로그인에 실패했다면, rs에 값이 없을 것		-> rs.next()로 값 조회X (적절한 값이 없음)
